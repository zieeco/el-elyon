...
export const scanOrphanedData = query({
	...
	handler: async (ctx) => {
		...
		const employeeClerkUserIds = new Set(
			allEmployees.map((e) => e.clerkUserId)
		);
		...
		for (const guardian of guardians) {
			if (guardian.residentIds && guardian.residentIds.length > 0) {
				const orphanedResidentIds = guardian.residentIds.filter(
					(rid) => !residentIds.has(rid)
				);
		...
		for (const kiosk of kiosks) {
			if (kiosk.createdBy && !employeeClerkUserIds.has(kiosk.createdBy)) {
				...
				kiosk.registeredBy &&
				!employeeClerkUserIds.has(kiosk.registeredBy)
			) {
				orphanedData.kiosks.push({
					id: kiosk._id,
					name: kiosk.name,
		...
		for (const employee of allEmployees) {
			const hasRole = roles.some(
				(role) => role.clerkUserId === employee.clerkUserId
			);
			if (!hasRole) {
		...
		// Calculate totals
		const totalOrphaned = Object.values(orphanedData).reduce(
...
export const cleanupOrphanedData = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (args.categories.includes('shifts')) {
			...
			for (const shift of shifts) {
				if (
					!employeeClerkUserIds.has(shift.clerkUserId) ||
					(shift.kioskId && !kioskIds.has(shift.kioskId))
				) {
		...
		if (args.categories.includes('residentLogs')) {
			...
			for (const log of residentLogs) {
				if (
					!residentIds.has(log.residentId) ||
					(log.authorId && !employeeClerkUserIds.has(log.authorId))
				) {
		...
		if (args.categories.includes('ispFiles')) {
			...
			for (const file of ispFiles) {
				if (
					!residentIds.has(file.residentId) ||
					!employeeClerkUserIds.has(file.uploadedBy)
				) {
		...
		if (args.categories.includes('ispAccessLogs')) {
			...
			for (const log of ispAccessLogs) {
				if (
					!ispFileIds.has(log.ispFileId) ||
					!residentIds.has(log.residentId) ||
					!employeeClerkUserIds.has(log.clerkUserId)
				) {
					await ctx.db.delete(log._id);
					count++;
					deletedCount++;
		...
		if (args.categories.includes('ispAcknowledgments')) {
			...
			for (const ack of ispAcks) {
				if (
					!residentIds.has(ack.residentId) ||
					!employeeClerkUserIds.has(ack.clerkUserId) ||
					!ispIds.has(ack.ispId)
				) {
		...
		if (args.categories.includes('guardians')) {
			...
			for (const guardian of guardians) {
				if (guardian.residentIds && guardian.residentIds.length > 0) {
					const validResidentIds = guardian.residentIds.filter((rid) =>
						residentIds.has(rid)
					);
		...
		if (args.categories.includes('employees')) {
			...
			const roleClerkUserIds = new Set(
				allRoles.map((role) => role.clerkUserId)
			);
			let count = 0;
			for (const employee of allEmployees) {
		...
		await audit(
			...
			clerkUserId,
			`categories=${args.categories.join(',')},deletedCount=${deletedCount}`
		);
