...
export const sendComplianceReminderEmails = internalAction({
	...
	handler: async (
		ctx,
		{itemIds, clerkUserId}
	): Promise<{sent: number; total: number; results: any[]}> => {
		...
		const items = await ctx.runQuery(
			internal.complianceEmails.getComplianceItemsForEmail,
			{
				itemIds,
			}
		);
		...
		const employee = await ctx.runQuery(
			internal.complianceEmails.getUserForEmail,
			{
		...
		const recipients: any[] = await ctx.runQuery(
			internal.complianceEmails.getComplianceRecipients,
			{}
		);
...
export const sendGuardianChecklistEmail = internalAction({
	...
	handler: async (ctx, {linkId, token}) => {
		const data = await ctx.runQuery(
			internal.guardianChecklists.internalGetChecklistLink,
			{linkId}
		);
    ...
    `;
		...
		const fromEmail =
			process.env.FROM_EMAIL || 'Care Team <noreply@care-team.example.com>';
	...
	handler: async (ctx, {itemIds}) => {
		...
		for (const itemId of itemIds) {
			...
			if (type === 'isp') {
				...
				if (activeISP) {
					const dueDate =
						activeISP.effectiveDate + 6 * 30 * 24 * 60 * 60 * 1000;
					const now = Date.now();
...
export const getComplianceRecipients = internalQuery({
	...
	handler: async (ctx) => {
		...
		for (const clerkUserId of adminAndSupervisorClerkUserIds) {
			// Changed from id of adminAndSupervisorIds
