...
// Helper: Check if user has care access (for queries - no audit)
async function requireCareAccessQuery(ctx: any, userId: Id<"users">) {
...
// Helper: Check if user has care access (for mutations - with audit)
...
export const getMyResidents = query({
  ...
  handler: async (ctx) => {
    ...
    const userRole = await requireCareAccessQuery(ctx, userId);
...
export const getLogTemplates = query({
  ...
  handler: async (ctx) => {
    ...
    await requireCareAccessQuery(ctx, userId);
